var tb = require('timebucket')
  ...
  , Ziggurat = require('node-ziggurat')
...
module.exports = function container (get, set, clear) {
  ...
  return function (s) {
    ...
    // for non-live trading modes, attempts to simulate actual asset price
    // will be very slightly skewed towards 0 due to rounding of negative slippage
    function simulateExecutionPrice () {

      if (s.mode === 'live' || so.avg_slippage_pct == 0)
        if (s.buy_order)
          return s.buy_order.price
        if (s.sell_order)
          return s.sell_order.price

      // generate a normally-distributed random seed
      // mean of dist is 0, standard deviation is 1
      var z = new Ziggurat()
      var seed = z.nextGaussian()

      // scale the seed to match the configured avg_slippage_pct, do this
      // by making sure that 3 standard deviations fit within 0 and the slippage
      var slippage = Math.max(n(so.avg_slippage_pct).add(n(so.avg_slippage_pct).divide(3).multiply(seed), 0)
)
      // factor estimated slippage into order price
      if (s.buy_order)
        return n(s.buy_order.price).add(n(s.buy_order.price).multiply(slippage / 100)).format('0.00000000')
      else if (s.sell_order)
        return n(s.sell_order.price).subtract(n(s.sell_order.price).multiply(slippage / 100)).format('0.00000000')
    }
